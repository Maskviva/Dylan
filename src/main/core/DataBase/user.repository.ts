...
export class userService {
     ...
     */
    public async addUser(name: string, email: string, password: string): Promise<ReturnType> {
        let connection: mysql.PoolConnection | undefined;
        try {
            // 尝试从连接池获取一个连接
            connection = await this.pool.getConnection();
            // 执行插入操作并获取结果
            const [result] = await connection.execute('INSERT INTO userInfo (name, email, password, portrait) VALUES (?, ?, ?, ?)', [name, email, password, '/assets/images/DefaultPortrait.png']);
            // 返回操作成功的信息和结果
     ...
     * 按 ID 更新用户信息
     *
     *  该方法尝试从数据库连接池获取一个连接 然后根据提供的用户 ID 和更新信息执行更新操作
     ...
     * @return {Promise<ReturnType>} 返回一个状态消息
     */
    public async updateUser(email: string, updates: Partial<userDataBase>): Promise<ReturnType> {
        let connection: mysql.PoolConnection | undefined;
        try {
            // 尝试从连接池获取一个连接
            connection = await this.pool.getConnection();
            ...
            // 拼接 SQL 语句的参数
            const params: (string | number | null)[] = [...values, email];
     ...
     * @param column    - 要查询的列名
     * @param value     - 要匹配列的值
     * @returns {Promise<[userDataBase] | undefined>} 返回一个 Promise 该 Promise 解析为包含用户信息的数组 如果未找到用户则返回 undefined
     */
    public async getUser(column: 'name' | 'email' | 'cookie', value: string): Promise<[userDataBase] | undefined> {
        let connection: mysql.PoolConnection | undefined;
        try {
            // 尝试从连接池获取一个连接
            connection = await this.pool.getConnection();
            ...
            return Array.isArray(rows) && rows.length > 0 ? (rows as [userDataBase]) : undefined;
        } catch (err) {
            // 捕获并记录错误信息
     ...
     */
    public async deleteUser(column: string | 'name' | 'email' | 'cookie', value: string | number): Promise<boolean> {
        let connection: mysql.PoolConnection | undefined;
        try {
            // 尝试从连接池获取一个连接
            connection = await this.pool.getConnection();
